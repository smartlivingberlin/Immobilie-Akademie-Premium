...
export function registerOwnerRoutes(app: Express) {
  ...
  // ── TESTER-ZUGANG: OTP verifizieren + Session erstellen ──────
  app.post("/api/tester/verify", async (req: Request, res: Response) => {
    ...
    if (!valid) return res.status(401).json({ error: "Ungültiger oder abgelaufener Code" });
    try {
      ...
      res.cookie("tester_expires", expiresAt.toISOString(), { 
