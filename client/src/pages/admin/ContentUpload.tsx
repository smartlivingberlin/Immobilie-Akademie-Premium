  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: fz(24), fontWeight: 700, color: "#0f172a", margin: 0 }}>Content Upload & KI-Analyse</h1>
      ...
      <div
        ...
        <div style={{ fontSize: fz(15), fontWeight: 600, color: "#374151", marginBottom: 4 }}>Dateien hierher ziehen oder klicken</div>
        <div style={{ fontSize: fz(12), color: "#94a3b8" }}>PDF · DOCX · PPTX · TXT · MP3 · WAV · PNG · JPG</div>
      ...
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
        <div style={{ fontSize: fz(12), fontWeight: 600, color: "#374151", marginBottom: 10 }}>Fragen speichern in Modul:</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          {[1,2,3,4,5].map(m => (
            <button key={m} onClick={() => setSelectedModule(m)}
              style={{ padding: "6px 14px", borderRadius: 20, border: "0.5px solid", fontSize: fz(12), fontWeight: 500, cursor: "pointer",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: fz(12), color: "#64748b" }}>Anzahl Fragen pro Analyse:</span>
          {[10, 15, 20, 25].map(n => (
            <button key={n} onClick={() => setQuestionCount(n)}
              style={{ padding: "4px 10px", borderRadius: 20, border: "0.5px solid", fontSize: fz(11), fontWeight: 500, cursor: "pointer",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 20 }}>
        {[
          ...
          <div key={f.label} style={{ padding: "10px 12px", background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10 }}>
            <div style={{ fontSize: fz(11), fontWeight: 600, color: f.color, marginBottom: 3 }}>{f.label}</div>
            <div style={{ fontSize: fz(11), color: "#64748b" }}>{f.items}</div>
          </div>
        ))}
      </div>

      {error && (
        <div style={{ display: "flex", gap: 8, padding: "12px 14px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, marginBottom: 12, color: "#991b1b", fontSize: fz(13) }}>
      ...
      {genResults.map((r, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, marginBottom: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: fz(13), fontWeight: 600, color: "#166534" }}>{r.saved} neue Prüfungsfragen gespeichert</div>
            <div style={{ fontSize: fz(11), color: "#4ade80" }}>aus "{r.filename}" → Modul {r.moduleId} ({MODULE_NAMES[r.moduleId]}) · sofort im Prüfungsmodus verfügbar</div>
          </div>
      ...
      {files.length > 0 && (
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, marginBottom: 16 }}>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid #f1f5f9", fontSize: fz(13), fontWeight: 600, color: "#374151" }}>
          ...
          {files.map(f => (
            <div key={f.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderBottom: "0.5px solid #f8fafc" }}>
              {getIcon(f.name)}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: fz(13), fontWeight: 500, color: "#374151" }}>{f.name}</div>
                <div style={{ fontSize: fz(11), color: "#94a3b8" }}>{(f.size / 1024).toFixed(0)} KB</div>
              </div>
              <button onClick={() => analyze(f)} disabled={!!loading}
                style={{ padding: "6px 14px", background: "#2563eb", color: "#fff", border: "none", borderRadius: 8, fontSize: fz(12), cursor: "pointer", display: "flex", alignItems: "center", gap: 6, opacity: loading ? 0.6 : 1 }}>
      ...
      {results.map((r, i) => (
        <div key={i} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", borderBottom: "1px solid #f1f5f9" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: fz(13), fontWeight: 600, color: "#374151" }}>{r.filename}</div>
              <div style={{ fontSize: fz(11), color: "#94a3b8" }}>{r.textLength.toLocaleString()} Zeichen analysiert</div>
            </div>
            <span style={{ padding: "3px 10px", background: "#f0fdf4", borderRadius: 20, fontSize: fz(11), color: "#166534", fontWeight: 500 }}>Analysiert</span>
          </div>
          <div style={{ padding: "16px", fontSize: fz(13), color: "#374151", lineHeight: 1.7, whiteSpace: "pre-wrap", maxHeight: 350, overflowY: "auto" }}>
            {r.analysis}
          </div>
          {/* Fragen-Generator */}
          <div style={{ padding: "12px 16px", borderTop: "1px solid #f1f5f9", background: "#fafafa", borderRadius: "0 0 12px 12px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontSize: fz(12), color: "#374151", flex: 1 }}>
              ...
              <button onClick={() => generateQuestions(r.analysis, r.filename)} disabled={!!genLoading}
                style={{ padding: "7px 16px", background: "#7c3aed", color: "#fff", border: "none", borderRadius: 8, fontSize: fz(12), fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, opacity: genLoading ? 0.6 : 1 }}>
      ...
      {files.length === 0 && results.length === 0 && genResults.length === 0 && (
        <div style={{ textAlign: "center", padding: "48px 20px", color: "#94a3b8" }}>
          ...
          <div style={{ fontSize: fz(14), fontWeight: 500 }}>Noch keine Dateien hochgeladen</div>
          <div style={{ fontSize: fz(12), marginTop: 6, maxWidth: 400, margin: "6px auto 0" }}>
