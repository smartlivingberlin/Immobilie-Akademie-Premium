  ...
  return (
    <div style={{ maxWidth: 950, margin: "0 auto", padding: "24px 20px" }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: fz(24), fontWeight: 700, color: "#0f172a", margin: 0 }}>Mediaskript-Generator</h1>
        <p style={{ color: "#64748b", marginTop: 4, fontSize: fz(14) }}>KI schreibt optimierte Skripte für NotebookLM-Podcast, echte Video-Aufnahmen und KI-Avatar-Videos</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        {/* Modul */}
        <div style={{ background: "#fff", border: "0.5px solid #e2e8f0", borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: fz(11), fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>1. Modul wählen</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {MODULES.map(m => (
              <div key={m.id} onClick={() => { setModuleId(m.id); setThema(""); }}
                ...
                <div style={{ width: 22, height: 22, borderRadius: 5, background: moduleId === m.id ? "#2563eb" : "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: fz(10), fontWeight: 700, color: moduleId === m.id ? "#fff" : "#64748b", flexShrink: 0 }}>M{m.id}</div>
                <span style={{ fontSize: fz(12), color: moduleId === m.id ? "#1d4ed8" : "#374151" }}>{m.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Thema */}
        <div style={{ background: "#fff", border: "0.5px solid #e2e8f0", borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: fz(11), fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>2. Thema wählen</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 8 }}>
            {(THEMEN[moduleId] || []).map(t => (
              <div key={t} onClick={() => { setThema(t); setCustomThema(""); }}
                style={{ padding: "6px 10px", borderRadius: 7, cursor: "pointer", fontSize: fz(12),
          ...
          <input
            ...
            style={{ width: "100%", padding: "7px 10px", border: "0.5px solid #e2e8f0", borderRadius: 7, fontSize: fz(12) }}
          />
        </div>
      </div>
      ...
      <div style={{ background: "#fff", border: "0.5px solid #e2e8f0", borderRadius: 12, padding: 14, marginBottom: 14 }}>
        <div style={{ fontSize: fz(11), fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 10 }}>3. Ausgabeformat wählen</div>
          ...
          {FORMATE.map(f => (
            <div key={f.id} onClick={() => setFormat(f.id)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                ...
                <span style={{ fontSize: fz(11), fontWeight: 600, color: format === f.id ? f.color : "#374151" }}>{f.label}</span>
              </div>
              <div style={{ fontSize: fz(10), color: "#94a3b8", lineHeight: 1.4 }}>{f.desc}</div>
              <div style={{ marginTop: 6, fontSize: fz(9), padding: "1px 6px", background: f.color + "20", color: f.color, borderRadius: 10, display: "inline-block", fontWeight: 500 }}>{f.badge}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <button onClick={generate} disabled={loading || !aktivesThema}
        style={{ width: "100%", padding: 14, background: loading || !aktivesThema ? "#94a3b8" : FORMAT_COLORS[format] || "#7c3aed", color: "#fff", border: "none", borderRadius: 12, fontSize: fz(14), fontWeight: 600, cursor: loading || !aktivesThema ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 20 }}>
      ...
      {result && (
        <div style={{ background: "#fff", border: "0.5px solid #e2e8f0", borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", borderBottom: "0.5px solid #f1f5f9", background: "#f8fafc" }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: fz(13), fontWeight: 600, color: "#374151" }}>{result.thema}</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={copy}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: copied ? "#059669" : "#2563eb", color: "#fff", border: "none", borderRadius: 8, fontSize: fz(12), cursor: "pointer" }}>
                {copied ? <CheckCircle size={13} /> : <Copy size={13} />}
                {copied ? "Kopiert!" : "Kopieren"}
              </button>
              <button onClick={download}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "#374151", color: "#fff", border: "none", borderRadius: 8, fontSize: fz(12), cursor: "pointer" }}>
                <Download size={13} /> Download
              </button>
              {result.format === "podcast" && (
                <a href="https://notebooklm.google.com" target="_blank" rel="noopener noreferrer"
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "#7c3aed", color: "#fff", borderRadius: 8, fontSize: fz(12), textDecoration: "none" }}>
          ...
          {result.format === "podcast" && (
            <div style={{ padding: "10px 16px", background: "#faf5ff", borderBottom: "0.5px solid #e9d5ff", fontSize: fz(12), color: "#6d28d9" }}>
          ...
          {result.format === "videoskript" && (
            <div style={{ padding: "10px 16px", background: "#fef2f2", borderBottom: "0.5px solid #fecaca", fontSize: fz(12), color: "#dc2626" }}>
          ...
          {result.format === "synthesia" && (
            <div style={{ padding: "10px 16px", background: "#f0fdf4", borderBottom: "0.5px solid #bbf7d0", fontSize: fz(12), color: "#059669" }}>
          ...
          {/* Vollbild */}
            <div style={{ padding: "8px 16px", borderBottom: "0.5px solid #f1f5f9" }}>
              <FullscreenContent
                ...
                content={<div style={{ whiteSpace: "pre-wrap", lineHeight: 1.9, fontSize: fz(15), fontFamily: "monospace" }}>{result.skript}</div>}
              />
            </div>
            {/* Skript-Text */}
          <div style={{ padding: 20, fontSize: fz(13), color: "#374151", lineHeight: 1.9, whiteSpace: "pre-wrap", maxHeight: 600, overflowY: "auto", fontFamily: "monospace, monospace" }}>
            {result.skript}
          </div>
        </div>
      )}
