...
export default function ReferralAdmin() {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: "24px 20px" }}>
      ...
      <h1 style={{ fontSize: fz(24), fontWeight: 800, color: "#0f172a", marginBottom: 8 }}>Empfehlungsprogramm</h1>
      ...
      {loading ? (
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
            {[
              ...
              <div key={label} style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 12, padding: 16 }}>
                ...
                <div style={{ fontSize: fz(22), fontWeight: 800, color: "#0f172a" }}>{value}</div>
                <div style={{ fontSize: fz(12), color: "#64748b" }}>{label}</div>
              </div>
            ))}
          </div>

          <h2 style={{ fontSize: fz(16), fontWeight: 700, marginBottom: 12 }}>Top-Empfehler</h2>
          {stats.topReferrers.length === 0 ? (
            <p style={{ fontSize: fz(13), color: "#94a3b8", marginBottom: 24 }}>Noch keine Empfehlungen mit Kauf.</p>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 28, fontSize: fz(13) }}>
              <thead>
              ...
              <tbody>
                {stats.topReferrers.map((r) => (
                  <tr key={r.userId} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    <td style={{ padding: "10px 12px" }}>{r.name}<br /><span style={{ color: "#94a3b8", fontSize: fz(11) }}>{r.email}</span></td>
          ...
          <h2 style={{ fontSize: fz(16), fontWeight: 700, marginBottom: 12 }}>Letzte Rewards</h2>
          {stats.recentRewards.length === 0 ? (
            <p style={{ fontSize: fz(13), color: "#94a3b8", marginBottom: 28 }}>Keine Rewards protokolliert.</p>
          ) : (
            <div style={{ marginBottom: 28, fontSize: fz(13) }}>
      ...
      <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20, marginBottom: 20 }}>
        <h2 style={{ fontSize: fz(16), fontWeight: 700, marginBottom: 12 }}>Auszahlungs-Ledger</h2>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          <button
            onClick={generateLedger}
            disabled={ledgerLoading}
            style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#2563eb", color: "white", cursor: "pointer", fontSize: fz(13), fontWeight: 600 }}
          >
          ...
          <button
            ...
            disabled={ledgerLoading}
            style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid #7c3aed", background: "white", color: "#7c3aed", cursor: "pointer", fontSize: fz(13), fontWeight: 600 }}
          >
            Alle Connect-fähigen auszahlen
          </button>
        </div>
        {ledger.length === 0 ? (
          <p style={{ fontSize: fz(13), color: "#94a3b8" }}>Noch keine Ledger-Einträge (Migration 0039).</p>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: fz(13) }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #e2e8f0", textAlign: "left" }}>
            ...
            <tbody>
              {ledger.map((r) => (
                <tr key={r.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={{ padding: "8px" }}>{r.partnerName}<br /><span style={{ color: "#94a3b8", fontSize: fz(11) }}>{r.partnerEmail}</span></td>
                  ...
                  <td style={{ padding: "8px" }}>
                    {r.status === "pending" && (
                      <span style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        <button onClick={() => connectTransfer(r.id)} style={{ fontSize: fz(11), padding: "4px 8px", borderRadius: 6, border: "1px solid #7c3aed", background: "white", cursor: "pointer", color: "#7c3aed" }}>
                          Connect
                        </button>
                        <button onClick={() => markPaid(r.id)} style={{ fontSize: fz(11), padding: "4px 8px", borderRadius: 6, border: "1px solid #059669", background: "white", cursor: "pointer", color: "#059669" }}>
                          Manuell
                        </button>
                      </span>
                    )}
                    {r.stripeTransferId && (
                      <span style={{ fontSize: fz(10), color: "#94a3b8", fontFamily: "monospace" }}>{r.stripeTransferId.slice(0, 12)}…</span>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {payoutDetails.length > 0 && (
        <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20, marginBottom: 20 }}>
          <h2 style={{ fontSize: fz(16), fontWeight: 700, marginBottom: 12 }}>Hinterlegte Auszahlungsdaten</h2>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: fz(13) }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #e2e8f0", textAlign: "left" }}>
                <th style={{ padding: "8px" }}>Partner</th>
            ...
            <tbody>
              {payoutDetails.map((r) => (
                <tr key={r.email} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={{ padding: "8px" }}>{r.name}<br /><span style={{ color: "#94a3b8", fontSize: fz(11) }}>{r.email}</span></td>
      ...
      <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: 20, marginBottom: 20 }}>
        <h2 style={{ fontSize: fz(16), fontWeight: 700, color: "#166534", marginBottom: 8, display: "flex", alignItems: "center", gap: 8 }}>
          <Banknote size={18} /> Partner-Auszahlungen
        </h2>
        <p style={{ fontSize: fz(13), color: "#166534", marginBottom: 12 }}>
        ...
        <a
          ...
          style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, background: "#059669", color: "white", fontSize: fz(13), fontWeight: 600, textDecoration: "none" }}
        >
      ...
      <div style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: 12, padding: 20 }}>
        <h2 style={{ fontSize: fz(16), fontWeight: 700, color: "#92400e", marginBottom: 8 }}>Backfill accessExpiresAt</h2>
        <p style={{ fontSize: fz(13), color: "#78350f", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button
            ...
            style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid #d97706", background: "white", cursor: "pointer", fontSize: fz(13) }}
          >
          ...
          <button
            ...
            disabled={backfillLoading}
            style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#d97706", color: "white", cursor: "pointer", fontSize: fz(13), fontWeight: 600 }}
          >
            Anwenden
          </button>
        </div>
        {backfill && (
          <div style={{ marginTop: 16, fontSize: fz(12), color: "#78350f" }}>
