...
export default function ModuleGuard({ moduleId, children }: ModuleGuardProps) {
  ...
  const hasAccess =
    user.role === "admin" || enabled.includes(String(moduleId));

  if (hasAccess) {
    return <>{children}</>;
  }

  // Modul gesperrt → Modal anzeigen
  ...
  return (
    <>
      {/* Hintergrund — verschwommene Preview des gesperrten Moduls */}
        ...
        <div className="relative z-10 flex items-center justify-center min-h-[70vh] px-4">
          ...
          >
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-2">
              Modul {moduleId} gesperrt
            </h2>
            <p className="text-slate-500 text-sm mb-1 font-medium">
              {moduleNames[moduleId] || `Modul ${moduleId}`}
            </p>
            <p className="text-slate-400 text-sm mb-6">
              Schalte dieses Modul frei, um auf alle Lerninhalte,
              Praxisrechner und den KI-Tutor zuzugreifen.
            </p>
